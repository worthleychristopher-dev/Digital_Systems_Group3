package color_data is
	...
	subtype color_index_type is natural range 0 to 3;
	type color_table_type is array(color_index_type) of rgb_color;

	subtype palette_index_type is natural range 0 to 2;
	type color_palette_type is array(palette_index_type) of color_table_type;

	constant color_table_1: color_table_type := (
			0 => ( red =>  0, green =>  0, blue => 15 ),
			1 => ( red =>  5, green =>  5, blue => 10 ),
			2 => ( red => 10, green => 10, blue =>  5 ),
			3 => ( red => 15, green => 15, blue =>  0 )
		);

	constant color_table_2: color_table_type := (
			0 => ( red => 15, green =>  0, blue =>  0 ),
			1 => ( red => 10, green =>  5, blue =>  0 ),
			2 => ( red =>  5, green => 10, blue =>  0 ),
			3 => ( red =>  0, green => 15, blue =>  0 )
		);
